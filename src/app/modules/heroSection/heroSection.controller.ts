...
const getAllHeroSection = catchAsync(
  async(req,res)=>{
...
const getSingleHeroSection = catchAsync(
  async(req,res)=>{
    const {id} = req.params;
...
const updateHeroSection = catchAsync(
  async(req,res)=>{
    const {id} = req.params; 
    const HeroSection = req.body; 
    const result = await HeroSectionServices.updateHeroSectionFromDB(id,HeroSection); 
    if (!result) {
