...
const createVoiceOnMedia = catchAsync(
  async(req,res)=>{
...
const getAllVoiceOnMedias = catchAsync(
  async(req,res)=>{
...
const getSingleVoiceOnMedia = catchAsync(
  async(req,res)=>{
    const {id} = req.params;  
...
const updateVoiceOnMedia = catchAsync(
  async(req,res)=>{
    const {id} = req.params; 
    const voiceOnMedia = req.body; 
    const result = await VoiceOnMediaServices.updateVoiceOnMediaFromDB(id,voiceOnMedia); 
    if (!result) {
      throw new Error("VoiceOnMedia not found");
    }
    sendResponse(res,{
...
const deleteVoiceOnMedia = catchAsync(
  async(req,res)=>{
    const {id} = req.params; 
