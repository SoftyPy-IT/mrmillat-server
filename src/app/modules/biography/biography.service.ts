...
const updateBiographyFromDB = async(id:string,payload:Partial<TBiography>)=>{
const{items=[],...basicData} = payload; 
const item =items[0];
if(item.action==='add'){
    await Biography.findByIdAndUpdate(id,{
    $addToSet:{items:item}
  },{new:true})
}
...
const result = await Biography.findByIdAndUpdate(id,basicData,{new:true});
